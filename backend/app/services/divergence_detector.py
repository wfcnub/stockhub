...
class DivergenceDetectorService:
    ...
    def _is_event_invalidated(
        self,
        points: List[PriceRsiPoint],
        event: Dict[str, object],
    ) -> bool:
        strategy = str(event.get("strategy_type"))
        if strategy not in (STRATEGY_AGGRESSIVE, STRATEGY_EMERGING, STRATEGY_CONFIRMED):
            return False
        ...
        for point in points:
            if self._to_unix_seconds(point.date) <= signal_timestamp:
                continue
            if point.low < invalidation_level:
                return True
    ...
    def detect_recent_market_divergences(
        ...
        for ticker in tickers:
            ...
            for event in ticker_events:
                is_invalidated = self._is_event_invalidated(points, event)
